...
<?php
    ...
    //1~20 --> 1페이지      : DESC 0, 20    --> ($viewNum * 1) - $viewNum
    //21~40 --> 2페이지     : DESC 20, 20   --> ($viewNum * 2) - $viewNum
    //41~60 --> 3페이지     : DESC 40, 20   --> ($viewNum * 3) - $viewNum
    //61~80 --> 4페이지     : DESC 60, 20   --> ($viewNum * 4) - $viewNum

    // 두개의 테이블을 join
    $sql = "SELECT b.myBoardID, b.boardTitle, m.youName, b.regTime, b.boardView FROM myBoard b JOIN myMember m ON (b.myMemberID = m.myMemberID) ORDER BY myBoardID DESC LIMIT {$viewLimit}, {$viewNum}"; //LIMIT로 게시물이 나오는 수를 정할 수 있음 DESC는 역순(최신글순)
    $result = $connect -> query($sql);
...
<?php
    ...
    if($page != $endPage){
        $naxtPage = $page + 1;
        echo "<li><a href='board.php?page={$naxtPage}'>다음</a></li>";
