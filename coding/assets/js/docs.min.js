...
!(function (a, b) {
  ...
  function g(a) {
    ...
    return (
      q / l >= 0.75 && (k = Math.floor(0.75 * k * (l / q))),
    ...
    (h = i ? e(h, { text: i }) : h),
      ...
      "image" == a
        ? (b.setAttribute("alt", i ? i : h.text ? h.text + " [" + j + "]" : j),
          ...
          n
            ? (b.style.backgroundColor = h.background)
            : (b.setAttribute(
                "src",
                g({ ctx: s, dimensions: f, template: h, ratio: t, holder: c })
              ),
              c.textmode && "exact" == c.textmode && (r.push(b), k(b))))
        : "background" == a
        ? n ||
          ((b.style.backgroundImage =
            ...
            g({ ctx: s, dimensions: f, template: h, ratio: t, holder: c }) +
            ")"),
          (b.style.backgroundSize = f.width + "px " + f.height + "px"))
        : "fluid" == a &&
          (b.setAttribute("alt", i ? i : h.text ? h.text + " [" + j + "]" : j),
          (b.style.height =
            "%" == f.height.slice(-1) ? f.height : f.height + "px"),
    ...
    (a.run = function (b) {
      ...
      for (
        "string" == typeof c.images
          ...
          ? (g = c.images)
          : window.Node && c.images instanceof window.Node && (g = [c.images]),
      ...
      j ||
        ...
        c.nocss ||
          (j.styleSheet
            ? (j.styleSheet.cssText += c.stylesheet)
            : j.appendChild(document.createTextNode(c.stylesheet)));
      for (
        var k = new RegExp(c.domain + '/(.*?)"?\\)'), n = i.length, o = 0;
        n > o;
        o++
      ) {
        var p = window
            .getComputedStyle(i[o], null)
        ...
        if (q) {
          ...
          var s = l(
            r.substr(r.lastIndexOf(c.domain) + c.domain.length + 1).split("/"),
            c
          );
          s && h("background", i[o], s, p);
        }
      ...
      for (n = f.length, o = 0; n > o; o++) {
        ...
        if (
          (null == attr_datasrc && v && v.indexOf(c.domain) >= 0
            ? (p = v)
            : attr_datasrc &&
          ...
          var s = l(
            p.substr(p.lastIndexOf(c.domain) + c.domain.length + 1).split("/"),
            c
          );
